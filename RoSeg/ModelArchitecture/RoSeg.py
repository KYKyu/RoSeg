...
class ConvBNAct(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, activation='relu', padding='same'):
        ...
        # padding 계산 (PyTorch는 'same' 자동 지원 안함)
        if padding == 'same':
    ...
    def _init_weights(self):
        nn.init.xavier_uniform_(self.conv.weight)  # TensorFlow와 동일한 초기화
...
class RoSeg(nn.Module):
    def __init__(self, input_channels, out_classes, starting_filters):
        ...
        # CAFormer backbone (use pretrained PyTorch version or feature extractor)
        # drop_path_rate = 0.1, norm_cfg = True (True for SyncBN, False for BatchNorm)
    ...
    def init_weights(self, pretrained=None):
        ...
        # pX_conv 레이어들에 대한 Glorot/Xavier 초기화 추가
        # Conv2d에 bias가 있는 경우, bias도 0으로 초기화
        ...
        # downX 레이어들에 대한 Glorot/Xavier 초기화 추가
        for m in [self.down1, self.down2, self.down3, self.down4, self.down5]:
            nn.init.xavier_uniform_(m.weight)
            if m.bias is not None: # Conv2d의 bias는 기본값으로 True
                nn.init.zeros_(m.bias)

        # self.final_conv는 이미 bias=True가 기본이므로, bias 초기화도 고려
...
# 모델 구조 확인용으로 사용
# def main():    
#     model = RoSeg(input_channels=3, out_classes=1, starting_filters=16)
#     dummy_input = torch.randn(1, 3, 512, 512)  # torch tensor (B, C, H, W)
#     summary(model, input_data=dummy_input, device='cpu')

# if __name__ == "__main__":
#     main()
# CustomLayer import 하는 부분 바꾸고 위의 주석을 푼 뒤에 terminal에서 
# python -m RAPUNet-main.ModelArchitecture.RAPUNet을 입력하면